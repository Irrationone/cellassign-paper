...
pdf("/datadrive/projects/cellassign-paper/drawings/v4/components/dr_plot.pdf", width = 3, height = 3)
dr_plot
dev.off()
```
...
pdf("/datadrive/projects/cellassign-paper/drawings/v4/components/probs.pdf", width = 5, height = 2.5, onefile = FALSE)
pheatmap(t(rbind(res$mle_params$gamma[sample(1:nrow(res$mle_params$gamma),18),], extra_rows)[sample(1:20, size = 20),]), legend = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, border_color = NA, show_colnames = FALSE, show_rownames = FALSE)
dev.off()
```
...
pheat_pal <- colorRampPalette(rev(brewer.pal(n = 7, 
    name = "RdYlBu")))(100)

prob_legend <- cellassign.utils::extract_legend(cellassign.utils::ggsimplelegend(vars = c(0, 1), colour_mapping = pheat_pal, legend_title = "Probability of cell type assignment", type = "continuous") + 
  theme(legend.key.width = unit(2, "lines")) + scale_fill_gradientn(colours = pheat_pal, limits = c(0,1), breaks = c(0,1)))

pdf("/datadrive/projects/cellassign-paper/drawings/v4/components/probs_legend.pdf", width = 6, height = 2, onefile = FALSE)
grid.draw(prob_legend)
dev.off()
```
...
```{r, echo = FALSE}
marker_binary_plot <- ggplot(example_rho_df, aes(x=celltype, y=gene)) + 
...
pdf("/datadrive/projects/cellassign-paper/drawings/v4/components/marker_plot.pdf", width = 6, height = 3)
marker_binary_plot
dev.off()
```
