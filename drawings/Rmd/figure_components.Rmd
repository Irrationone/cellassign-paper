...
dr_plot <- plotUMAP(sce_sim, ncomponents = 2, colour_by = "cluster", add_ticks = FALSE) + 
  guides(fill = FALSE) + 
  xlab("") + 
  ylab("") +
...
pdf("/datadrive/projects/cellassign-paper/drawings/components/dr_plot.pdf", width = 3, height = 3)
dr_plot
dev.off()
```
...
pdf("/datadrive/projects/cellassign-paper/drawings/components/probs.pdf", width = 3, height = 5, onefile = FALSE)
pheatmap(res$mle_params$gamma[sample(1:nrow(res$mle_params$gamma),20),], legend = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, border_color = NA, show_colnames = FALSE)
dev.off()
```
...
pdf("/datadrive/projects/cellassign-paper/drawings/components/marker_plot.pdf", width = 6, height = 3)
marker_binary_plot
dev.off()
```
