...
mean_percentile <- as.numeric(params$mean_percentile)
...
markers_to_use <- select_markers(sce_sim, percentile_fc = fc_percentile, percentile_meanexpr = mean_percentile, frac_genes = frac_genes, max_genes = max_genes)
...
if (clustering_method == "cellassign") {
  ...
  sce_sim_full <- cluster_wrapper(sce_sim, gene_subset = NULL, dimreduce_method = dimreduce_method, clustering_method = clustering_method)
  plotPCA(sce_sim_full, colour_by = "cluster")
  plotTSNE(sce_sim_full, colour_by = "cluster")
  
  print(with(sce_sim_full, table(cluster, Group)))
  evaluation_measures_full <- map_and_evaluate_clusters(sce_sim_full)
  
  sce_sim_markers <- cluster_wrapper(sce_sim, gene_subset = markers_to_use, dimreduce_method = dimreduce_method, clustering_method = clustering_method)
  plotPCA(sce_sim_markers, colour_by = "cluster")
  plotTSNE(sce_sim_markers, colour_by = "cluster")
  
  print(with(sce_sim_markers, table(cluster, Group)))
