...
sce_malignant <- sce_bcell %>% 
  scater::filter(malignant_status_manual == "malignant")

hla_genes <- c("HLA-A", "HLA-B", "HLA-C", "CD74", "HLA-DRA", "HLA-DRB1")
...
hla_exprs <- logcounts(sce_malignant)[hla_ids,] %>%
  as.matrix %>%
  t
colnames(hla_exprs) <- hla_genes
hla_exprs_annotated <- colData(sce_malignant) %>%
  data.frame(check.names = FALSE) %>%
  cbind(hla_exprs) %>%
  reshape2::melt(
...
hla_exprs_annotated_summary <- hla_exprs_annotated %>%
  dplyr::group_by(patient, timepoint, Symbol, logFC.T1) %>%
...
bottom_row <- cowplot::plot_grid(hla_lineplot, cycling_plot, ncol = 2, nrow = 1, labels = c('e', 'f'))
