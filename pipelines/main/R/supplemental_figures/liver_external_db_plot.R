# Liver external DB celltype plots
...
parser <- ArgumentParser(description = "Liver external DB celltype plots.")
...
parser$add_argument('--cellassign_fit_liver', metavar='FILE', type='character',
                    help="CellAssign fit to liver cell types")
...
model_fits <- list(
  'cellassign_fit_liver'=readRDS(args$cellassign_fit_liver)
)
...
celltype_plots <- lapply(seq_along(names(model_fits)), function(i) {
  ...
  sce_liver_subset$celltype[!sce_liver_subset$celltype %in% marker_types] <- "Unassigned"
...
# Plot final plot
pdf(args$outfname, width = 10, height = 6, useDingbats = FALSE)
plot(final_plot)
dev.off()
