# Differential expression between celltypes and epithelial groups
...
sce_filtered <- sce %>%
  scater::filter(celltype != "other") %>%
  scater::mutate(group = ifelse(celltype == "Epithelial cells", ifelse(epithelial_cluster == "1", 
                                                                       "Epithelial (1)",
...
scran_res <- de_analysis(sce_filtered, de_method = "scran", formula = NULL, cluster = sce_filtered$group, 
